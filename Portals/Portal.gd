...
# Sets the portal itself and collision to the script owner.
func _update_portal_area_size():
	$PortalVisual.scale.x = self.size.x
	$PortalVisual.scale.y = self.size.y
...
func _thicken_portal_if_necessary_to_prevent_camera_near_cull():
	...
	var half_portal_width = $PortalVisual.scale.x / 2.0
	var half_portal_height = $PortalVisual.scale.y / 2.0
	...
	$PortalVisual.scale.z = thickness
...
###### CAMERA ######
# All the code relating to the camera.
...
func _update_camera_to_other_portal():
	...
	var cur_camera_position_rel_to_this_portal = self.global_position * cur_camera.global_position
	#var cur_camera_rotation_rel_to_this_portal = self.global_rotation.affine_inverse() * cur_camera.global_transform
	#var cur_camera_rotation_rel_to_this_portal = self.global_rotation * cur_camera.global_transform
	var moved_to_other_portal = other_portal.global_position + cur_camera_position_rel_to_this_portal
	#var rotated_to_from_portal = other_portal.global_rotation + cur_camera_rotation_rel_to_this_portal
	
	# then, set the portal camera's transform to that relative position/rotation, but relative to other_portal
	var camView = $CameraViewport
	var cam3D = $CameraViewport/Camera3D
	
	#cam3D.global_transform = moved_to_other_portal
	cam3D.global_position = moved_to_other_portal
	#cam3D.global_rotation = rotated_to_from_portal
	cam3D.fov = cur_camera.fov
	
	cam3D.cull_mask = cur_camera.cull_mask
	cam3D.set_cull_mask_value(other_portal.cull_layer, false)
	
	camView.size = get_viewport().get_visible_rect().size
	camView.msaa_3d = get_viewport().msaa_3d
	camView.screen_space_aa = get_viewport().screen_space_aa
	camView.use_taa = get_viewport().use_taa
	camView.use_debanding = get_viewport().use_debanding
	camView.use_occlusion_culling = get_viewport().use_occlusion_culling
	camView.mesh_lod_threshold = get_viewport().mesh_lod_threshold
	
	_update_portal_camera_near_clip_plane(cam3D)
...
###### DUPELICATE MESH ######
# Duplicate the player's mesh to the other side of the portal.
# Making the the stand inside plane cut-off cosmetically non-existant.
...
func _try_detect_portal_pass_through_on_multiplayer_peer(tracked_body):
	if not tracked_body.mesh_duplicator:
		return false
	const DISTANCE_THRESHOLD_FROM_DUPLICATE = 3
...
func _move_to_other_portal(body: PhysicsBody3D):
	...
	## Teleport
	# Gets the transform based on this portal. So if you walk to the left of the doorframe youll come out to the left side.
	var transform_rel_to_this_portal = self.global_transform.affine_inverse() * (body.global_transform)
	#transform_rel_to_this_portal.rotation_degrees.y += 180
	# ??
