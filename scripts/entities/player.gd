...
@onready var backstab_system: BackstabSystem = GlobalReferences.backstab_system
@onready var checkpoint_system: CheckpointSystem = GlobalReferences.checkpoint_system


func _ready() -> void:
	GlobalReferences.lock_on_system.lock_on.connect(
		func(target: LockOnComponent): lock_on_target = target
	)
	
	GlobalReferences.dizzy_system.dizzy_victim_killed.connect(
		func(): instability_component.instability = 0.0
	)
	
	GlobalReferences.void_death_system.fallen_into_the_void.connect(
		func(body: Node3D):
