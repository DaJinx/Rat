...
func _ready() -> void:
	if patrol:
		...
		target = GlobalReferences.player
...
func _physics_process(_delta: float) -> void:
	...
	blackboard.get_value("rotate_towards_target"):
		if target == GlobalReferences.player:
			head_rotation_component.desired_target_pos = \
				GlobalReferences.player.lock_on_attachment_point.global_position
		else:
...
func switch_target(player: bool) -> void:
	if patrol and not player:
		...
		target = GlobalReferences.player as Node3D
	_set_target_values()
	_set_agent_values()
...
func _on_health_component_zero_health() -> void:
	...
	if GlobalReferences.backstab_system.backstab_victim == backstab_component:
		hit_and_death_animations.death_2()
	elif GlobalReferences.dizzy_system.dizzy_victim == dizzy_component and \
	...
	if blackboard.get_value("notice_state") == "aggro":
		GlobalReferences.music_system.fade_to_idle()
