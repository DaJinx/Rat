...
@onready var _player_melee_component: PlayerCombat = GlobalReferences.player.Combat
...
func _process(_delta) -> void:
	...
	_current_dist_to_player = backstab_victim\
		.entity\
		.global_position\
		.distance_to(GlobalReferences.player.global_position)
...
#	if backstab_victim: prints(victim.get_parent().name, backstab_victim.get_parent().name, dist, _current_dist_to_player)
	...
	var lock_on_system_target: LockOnComponent = GlobalReferences.lock_on_system.target
